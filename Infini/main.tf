...
module "ecs_fargate" {
  ...
  name             = "${local.tenant_name}-dev-task-definition"
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        },
        {
          ...
          value = "${local.tenant_name}-log-group"
        },
        {
          name  = "LOGGING_LOGGER_STREAMNAME"
          value = "${local.tenant_name}-log-steam-app"
        },
        {
  ...
  description                        = "This is ${local.tenant_name}-Dev"
  enabled                            = true
...
module "alb" {
  source                     = "../modules/alb"
  name                       = "${local.tenant_name}-dev-fargate-lb"
  vpc_id                     = module.vpc.vpc_id
  subnets                    = module.vpc.public_subnet_ids
...
terraform {
  backend "s3" {
    bucket = "tenant2-terraform-remote-state1"
